...
void manejar_sigusr1(){
	...
	char* ruta_completa = get_path_dump_tlb();
	char* ruta = string_from_format("DUMP_%s", timestamp);
	char* ruta_final = string_from_format("%s/%s", ruta_completa, ruta);

	loggear_info("[SIGUSR1] - Se va a generar el archivo dump en la ruta: %s", ruta_final);

	FILE* dump = fopen(ruta_final, "wb+");
